...
local function get_consumer_id(conf)
  ...
  if not consumer_id or consumer_id == null then
    consumer_id = EMPTY_UUID
  end
  return consumer_id
end
  ...
  ["cluster"] = {
    increment = function(conf, limits, identifier, current_timestamp, value)
      ...
      local ok, err = policy.increment(db.connector, limits, identifier,
                                       current_timestamp, service_id, route_id, consumer_id,
                                       value)
    ...
    usage = function(conf, identifier, period, current_timestamp)
      ...
      if row and row.value ~= nil and row.value > 0 then
        return row.value
      end
    ...
    parent_config = function(parent_consumer_id) 
      local db = kong.db
      local policy = policy_cluster[db.strategy]

      local row, err = policy.find_parent_config(db.connector,parent_consumer_id)
